...
dishRouter.route('/')
.options(cors.corsWithOptions, (req, res) => { res.sendStatus(200)}) //pre-flighting available options using OPTIONS method
...
.post( cors.corsWithOptions, authenticate.verifyUser, authenticate.verifyAdmin, (req, res, next) => { // add authenticate.verifyUser to authenticate the user. If it fails, passport.authenticate will reply back to the client with the proper error msg
